...
def forks_query(self, repos):
    ...
    try:
        ...
        # logging.info("FORKS_GRAPH_QUERY - CONNECTED!!!!!!!!!!!!!!!!!!!!!")
    except KeyError:
    ...
    df["repo_id"] = df["repo_id"].str[:15]

    # change to compatible type and remove all data that has been incorrectly formated
    df["repo_added"] = pd.to_datetime(df["repo_added"], utc=True).dt.date
    # df = df[df.repo_added < dt.date.today()]
    ...
    for i, r in enumerate(repos):
        ...
        c_df = pd.DataFrame(df.loc[df["repo_id"] == r]).reset_index(drop=True)
